...
process.TPGParamProducer = cms.EDFilter("EcalTPGParamBuilder",
    ...
    outFile = cms.string('TPG_beamv3WithAPDGain10.txt'),
   #### TPG config tag and version (if not given it will be automatically given ) ####
    TPGtag = cms.string('BEAMV3WITHAPDGAIN10'),
    TPGversion = cms.uint32(1),
                                        
   #### TPG calculation parameters ####

    ##########################################################
    # Warning: APD Gain = 10 (i.e. nominal gain50/5)         #
    #  ==> All numbers in GeV below must be multiplied by 5  #
    #  ==> All numbers in ADC below must be divided by 5     # 
    ...
    Et_sat_EB = cms.double(320.0),            ## Saturation value (in GeV) of the TPG before the compressed-LUT (rem: with 35.84 the TPG_LSB = crystal_LSB)
    ...
    pedestal_offset =  cms.uint32(60),      ## pedestal offset used with option forcedPedestalValue = -3

    useInterCalibration = cms.bool(True),    ## use or not values from DB. If not, 1 is assumed

    SFGVB_Threshold = cms.uint32(10),        ## used with option forcedPedestalValue = -3
    ...
    LUT_threshold_EB = cms.double(1.250),    ## All Trigger Primitives <= threshold (in GeV) will be set to 0 
    ...
    TTF_lowThreshold_EB = cms.double(5.0),   ## EB Trigger Tower Flag low threshold in GeV
    TTF_highThreshold_EB = cms.double(10.0), ## EB Trigger Tower Flag high threshold in GeV
    ...
    FG_lowThreshold_EB = cms.double(19.5),     ## EB Fine Grain Et low threshold in GeV
    FG_highThreshold_EB = cms.double(19.5),    ## EB Fine Grain Et high threshold in GeV
